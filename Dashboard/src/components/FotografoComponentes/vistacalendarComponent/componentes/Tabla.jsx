...
const Tabla = ({
  ...
  const eventos = eventosEnCurso.concat(eventosFuturos).concat(eventosPasados);
  ...
  return (
    <div className="w-[100%] bg-white h-[746px] overflow-scroll mt-[16px] rounded-t-[10px] border border-zinc-200">
      ...
      <table className="w-[100%] flex flex-col h-[594px]">
        <thead className="w-[100%] h-[42px]">
          <tr className="flex items-center h-[42px]">
            <th className="text-[#494949] w-[23.48%] text-[12px] font-normal border-b-[1px] px-6 py-5 text-left">
              Club
            </th>
            <th className="text-[#494949] w-[16.23%] text-[12px] font-normal border-b-[1px] px-6 py-5 text-left">
              Fecha
            </th>
            <th className="text-[#494949] w-[13.29%] text-[12px] font-normal border-b-[1px] px-6 py-5 text-left">
              Hora
            </th>
            <th className="text-[#494949] w-[35.45%] text-[12px] font-normal border-b-[1px] px-6 py-5 text-left">
              Dirección
            </th>
            <th className="text-[#494949] w-[23.08%] text-[12px] font-normal border-b-[1px] px-6 py-5 text-left">
              Estado
            </th>
          </tr>
        </thead>
        <tbody className="w-[100%] min-h-[594px] overflow-y-hidden">
          {eventosDelMes &&
            eventosDelMes.map((ev) => {
              ...
              const inscriptoAlEvento = misEventos?.find(evento => evento?.id === ev?.id)

              return (
                <tr key={ev.id} className="flex items-start h-[92px]">
                  ...
                  <th className="w-[23.08%] text-[12px] border-b-[1px] px-6 py-5 text-left h-[92px] flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`${
                        ev.estado === "Pasado" && "bg-zinc-100 text-stone-400 border-transparent"
                      } ${
                        inscriptoAlEvento && "bg-zinc-100 text-stone-400 border-transparent"
                      } bg-white text-[#23254C]  w-[92px] h-[36px] rounded border border-[#23254C]`}
                      disabled={ev.estado === "Pasado" || inscriptoAlEvento ? true : false}
                    >
                      {inscriptoAlEvento ? 'Inscripto' : 'Inscribirse'}
                    </button>
                  </th>
                </tr>
              );
