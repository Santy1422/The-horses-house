...
const handleFilterDelete = (f) => {
  ...
  setfilterOptions(deletefiltrostate);
};

return (
  <div className="inline-flex items-center w-[75%] gap-1 ">
    <div className="flex flex-row mt-4 justify-start gap-1.5">
          ...
          filtros.map((f, i) => {
            return (
              <Button
                ...
                key={i}
              >
                <p className="text-indigo-950 w-full text-sm font-normal font-lato leading-tight">
                  {f}
                </p>
                <svg
                  width="21"
      ...
      <div className="relative flex flex-col">
        ...
        >
          ...
          {filtros.length ? (
            ...
            <p className="text-indigo-950 text-sm w-full font-normal font-lato leading-tight">
              Filtrar
            </p>
          )}
        </Button>

        {/* ventana de opciones de filtros */}
        {showFiltros ? (
          <div className="absolute right-1 z-50 w-[130%] h-auto mt-10  bg-white rounded-sm border border-black border-opacity-20 shadow flex flex-col">
            {options &&
              options.map((op, i) => {
                return (
