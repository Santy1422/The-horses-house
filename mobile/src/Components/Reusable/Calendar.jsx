...
const Calendar = ({birthDay, birth, setDate, myCalender}) => {
    ...
    const renderDays = () => {
        ...
        return [...emptyDaysArray, ...daysArray].map((day, index) => {
          ...
          return (
            <View key={index} className={`p-2 text-center w-5  justify-center items-center`}>
              {day !== null ? (
                <Button
    ...
    <View className={`flex-1 flex-col px-4 pt-8 ${myCalender ? "w-11/12 shadow-2xl shadow-black bg-white rounded-2xl " : ""}`}>
            {   show === "days" || show === "months" ?
                ...
                <View className="flex-row">
                    <Button onPress={()=>setShow("months")} label={months[month]} labelClass={"mr-2 font-poppinsSemiBold text-xl text-black"}/>
                    <Button onPress={()=>setShow("years")} label={year} labelClass={"font-poppinsSemiBold text-xl text-black"}/>
                </View>
