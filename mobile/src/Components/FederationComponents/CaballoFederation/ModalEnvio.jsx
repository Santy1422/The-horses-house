...
const ModalEnvio = ({mostrar, setMostrar, modalEnvioDatos, setModalEnvioDatos , receptorCredencial, setReceptorCredencial, setModalVisible, setModalEnvioConfirmacion}) => {
    ...
    const closeModal = () => {
        ...
        if (mostrar) {
            setMostrar({...mostrar, seleccionar: true, personalmente: false, correo: false, modalCorreo: false})
        }
    }
    ...
    const confirmarEnvio = () => {
        ...
        if (mostrar) {
            setMostrar({...mostrar, personalmente: false, correo: true, modalCorreo: false})
            setReceptorCredencial({...receptorCredencial, envioCorreo: true})
        }
    } 
      ...
      return (
        <ScrollView className="bg-white h-full">
            ...
            <Modal
                transparent={true}
                visible={modalEnvioDatos || mostrar?.modalCorreo != undefined}
                animationType="slide" 
                        ...
                        <View className='bg bg-white rounded-md w-full px-6 pt-6 h-[622px]'>
                            ...
                            <View className="datosPersonales flex flex-col ">
                                <View className='nombre'>
                                    <ReusableTextInput label={'Nombre de quien recibe'} value={receptorCredencial.nombreReceptor} setValue={(nombre) => setReceptorCredencial({...receptorCredencial, nombreReceptor: nombre})}/>
                                </View>
                                <View className='domicilio'>
                                    <ReusableTextInput label={'Domicilio'} value={receptorCredencial.domicilio} setValue={(domicilio) => setReceptorCredencial({...receptorCredencial, domicilio: domicilio})}/>
                                </View>
                                <View className='localidad'>
                                    <ReusableTextInput label={'Localidad'} value={receptorCredencial.localidad} setValue={(localidad) => setReceptorCredencial({...receptorCredencial, localidad: localidad})}/>
                                </View>
                                ...
                                <View className='postal'>
                                    <ReusableTextInput label={'Código postal'} value={receptorCredencial.codigopostal} setValue={(postal) => setReceptorCredencial({...receptorCredencial, codigopostal: postal})}/>
                                </View>
                            </View>
                
                            <View className="flex items-center flex-row mt-[42px] mb-[38px] ">
                                {!mostrar &&<ArrowFunction onPress={goBack} />}
                                <Button  label={mostrar ? 'Confirmar' : `Confirmar envío `} extra=" flex-1 h-11 ml-3" onPress={confirmarEnvio} /> 
                            </View> 
                        </View>
                    </BlurView>
                </View> 
