...
const PhoneInput = ({ setValue }) => {
    ...
    const [prefix, setPrefix] = useState([{US: '+1'}, {AR: '+54'}, {BR: '+55'}])
    ...
    useEffect(()=> {
      if (isValid) {
        ...
        setError('please enter a correct telephone')
        setValue('')
        setDivBlue(false)
      }     
    ...
    return (
        <div className="flex flex-col w-80 items-start gap-1.5">        
          <label className="w-10 h-5 font-primary">Phone Number</label>
          <div className={`flex w-80 h-11 px-3.5 py-2.5 bg-white border rounded items-center gap-2 ${divBlue ? "border-blue-300" : ''} ${error ? "border-red-300" : phone ? 'border-green-300' : ''}`}>
            <select 
                name="" 
                id=""         
            ...
            <div className={`w-11 ${prefixp ? '' : 'text-gray-400'}`}>{prefixp ? prefixp : "+1"}</div>
            <input
              type='tel'
              value={phone}
