...
const TextInput = ({value, setValue, placeholder, label, hintEditable, weight, height}) => {
    /*
    ...
    */
    ...
    return (
        <div className={`flex flex-col w-full  items-start gap-1.5 `}>        
          <label className="w-full h-5 font-lato text-base font-normal leading-6 text-[#23254C]">{label}</label>
          <div className={`flex h-11 px-3.5 py-2.5 bg-white border rounded gap-2  w-full h-[${height}] ${divBlue ? "border-blue-300" : ''} ${error ? "border-red-300" : text ? 'border-green-300' : ''}`}>
