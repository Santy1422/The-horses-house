...
const Feature = ({svg, text, supporting, ifMore, width, working, mb}) => {
    ...
    return (
        <div className={` flex-col justify-start items-center gap-4 inline-flex`} style={{width: width}}>
            <div className={`w-12 h-12 bg-slate-600 rounded-[28px] border-8 border-slate-500 justify-center items-center inline-flex ${ mb ? 'mb-[19px]' : '' }`}>
                {svg}
            </div>
            {!working &&  <div className="w-[117px] h-6 px-2.5 py-0.5 bg-black rounded-2xl justify-center items-center inline-flex opacity-70 mt-[-19px]">
                <div className="text-center text-white text-sm font-small">{t('upcoming')}</div>
            </div>}
            <div className="self-stretch flex-col justify-start items-center gap-2 flex">
                <p className={`self-stretch text-center text-indigo-950 text-xl font-bold ${styles["text-font-lato"]} leading-[30px]`}>{t(`${text}`)}</p>
                <p className={`self-stretch text-center text-slate-600 text-base font-normal ${styles["text-font-lato"]} leading-normal`}>{t(`${supporting}`)}</p>
            </div>
            {
                ifMore &&
